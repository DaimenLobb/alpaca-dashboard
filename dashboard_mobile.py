...
def calc_open_risk_from_visible_rows(rows):
    """Open Risk should reflect the cards currently shown on the dashboard."""
    total_positions = 0
    total_orders = 0
    for row in rows or []:
        try:
            if row.get("detail_only"):
                continue
            total_positions += int(float(row.get("positions", 0) or 0))
            total_orders += int(float(row.get("orders", 0) or 0))
        except Exception:
            pass
    return total_positions, total_orders
...
with s2:
    # Open Risk must use visible card rows only, not stale/raw sheet rows.
    try:
        open_positions, open_orders = calc_open_risk_from_visible_rows(rows)
        total_positions, total_orders = open_positions, open_orders
    except Exception:
        pass
